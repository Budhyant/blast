...
while idx_cnt < 2:
    ...
    if sum_tp_f[idx_cnt] == 0:
        ...
        #     print('T+ F (air):    ', tps)
        # else:
        #     print('T+ F (surface): ', tps)
    idx_cnt = idx_cnt + 1
# print('Tp', tp_arr)


# pass the param result
class GetParamsForPlot:
    # dimensions
    dim_b = 1
    dim_l = 1
    dim_h = 3.6
    ...
    def __init__(self):
        ...
        self.tc_arr = self.get_tc(u_arr)
        self.lw_arr = self.get_lw(u_arr, tp_arr)
        self.qo_arr = self.get_qo(ps_arr)
        self.p_stag_arr = self.get_p_stag(ps_arr)
        self.iw_arr = self.get_iw(tp_arr)
        self.te_arr = self.get_te(pr_arr)
        self.pc_arr = self.get_pc()

    def get_lw(self, u_arr, td_arr):
        length = len(u_arr)
        lw_arr = []
        for idx in range(0, length):
            lw = u_arr[idx] * td_arr[idx]
            lw_arr.append(lw)
        return lw_arr

    def get_tc(self, u_arr):
        tc_arr = []
        for u in u_arr:
            tc = 3 * self.s / u
    ...
    def get_p_stag(self, ps_arr):
        p_stag_arr = []
        for idx, ps in enumerate(ps_arr):
            p_stag = ps + self.c_d * self.qo_arr[idx]
            p_stag_arr.append(round(p_stag, 3))
        return p_stag_arr

    def get_iw(self, tp_arr):
        iw_arr = []
        length = len(self.pr_arr)
        for idx in range(0, length):
            iw = 0.5 * ((self.pr_arr[idx] - self.p_stag_arr[idx]) * self.tc_arr[idx] + self.p_stag_arr[idx] * tp_arr[idx])
            # print('iw', iw)
            iw_arr.append(round(iw, 3))
        return iw_arr

    def get_te(self, pr_arr):
        te_arr = []
        length = len(self.pr_arr)
        for idx in range(0, length):
